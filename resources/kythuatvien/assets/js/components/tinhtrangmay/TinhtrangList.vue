    ...
    <v-card>
      ...
      <v-data-table :headers="headers" :items="tinhtrangmaylist" :search="search" :rows-per-page-items="[10,20]" :loading="isLoading">
        ...
        <template slot="items" slot-scope="props">
          ...
          <td class="justify-center layout px-0" v-if="props.item.kythuatvien_id == kythuatvien_current_id">
            <v-btn icon class="mx-0" @click="editItem(props.item.id)" v-if="editInDay(props.item.ngayghinhan) && props.index == 0">
...
export default {
  ...
  methods: {
    ...
    viewItem (id) {
      this.$router.push({ path: `/kythuatvien/tinhtrangmay/${this.may_slug}/view/${id}` });
    },
    deleteItem (item,id) {
