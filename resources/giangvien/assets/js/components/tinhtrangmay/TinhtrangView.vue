      ...
      <v-card ref="form">
        ...
        <v-form v-model="valid" v-on:submit.prevent = "createtinhtrangmay" method="POST">
          <v-card-text>
            ...
            <v-text-field
              v-if="tinhtrangmay.mota_kythuatvien"
              name="input-7-2"
              label="Ghi nhận"
              value="Input Text"
              class="input-group--focused"
            ...
            <!-- <v-radio-group v-model="tinhtrangmay.tinhtrang" column label="Tình trạng hiện tại" disabled>
              <v-radio
                label="Tốt"
                value="0"
              ></v-radio>
          ...
          <v-card-actions>
            <v-btn flat :to="{ name: 'TinhtrangList' }">Back</v-btn>
          </v-card-actions>
        </v-form>
      </v-card>
    </v-flex>
 ...
 export default {
   data: function () {
     return {
        ...
        tinhtrangmay: {mota_kythuatvien: '', mota_giangvien: '', tinhtrang: '0'},
   ...
   created() {
      ...
      _this.may_slug = urlfullCurrent.slice(urlfullCurrent.lastIndexOf('tinhtrangmay/') + 13 , urlfullCurrent.indexOf('/view'));
      let tinhtrangmay_id = urlfullCurrent.slice(urlfullCurrent.lastIndexOf('view/') + 5 , urlfullCurrent.length);

      Axios.get(_this.urlCurrent+'/api/tinhtrangmay/'+ tinhtrangmay_id + '/edit').then((response) => {
        _this.tinhtrangmay.mota_kythuatvien = response.data.mota_kythuatvien;
        _this.tinhtrangmay.mota_giangvien = response.data.mota_giangvien;
        // console.log();
      });
   ...
   methods: {
     createtinhtrangmay: function() {
      ...
      //console.log(this.tinhtrangmay);
       let uri = location.origin + '/api/tinhtrangmay';
       Axios.post(uri, dataToTinhtrangmay).then((response) => {
