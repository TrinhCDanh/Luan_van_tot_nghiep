                ...
                <v-card ref="form">
                    ...
                    <v-form v-model="valid" v-on:submit.prevent="editphongmay" method="POST">
                        <v-card-text>
                            ...
                            <v-btn @click="showMayList(phongmay.id)" style="margin-left: 0; color: white" color="blue"
                                   light>Số lượng máy có trong phòng: {{phongmay.soluongmay}}
                            </v-btn>
                        </v-card-text>
...
<script>
    export default {
        ...
        created: function () {
            ...
            let phongmay_id = urlCurrent.slice(urlCurrent.lastIndexOf('edit/') + 5);
            let url = location.origin + '/api/phongmay/' + phongmay_id + '/edit';
