<template id="edit-lichtruc">
    <div>
        ...
        <v-form v-model="valid" v-on:submit.prevent="createlichtruc" method="POST">
            <v-layout row wrap>
                <v-flex xs12 sm4 md4>
                    <v-text-field v-model="lichtruc.tenhocky + ' - ' + lichtruc.namhoc" label="Học kỳ"
...
<script>
    export default {
        ...
        created() {
            ...
            let lichtruc_id = urlfullCurrent.slice(urlfullCurrent.lastIndexOf('edit/') + 5);
            axios.get(_this.urlCurrent + '/api/lichtruc/' + lichtruc_id + '/edit').then((rep) => {
                _this.lichtruc = rep.data[0];
                _this.selectedKythuatvien = {id: _this.lichtruc.kythuatvien_id, name: _this.lichtruc.name}
            })
            axios.get(_this.urlCurrent + '/api/kythuatvien/').then((rep) =>{
