<template id="tinthrangmay-list">
  <div class="row">
    <v-card>
      <v-card-title>
        <p class="display-1">Lịch sử ghi nhận của máy {{ sothutumay }}</p>
        <v-spacer></v-spacer>
        <v-text-field
          v-model="search"
          append-icon="search"
      ...
      <v-data-table :headers="headers" :items="tinhtrangmaylist" :search="search" :rows-per-page-items='[10, 20, 30, { text: "All", value: -1 }]' :loading="isLoading">
    ...
    <v-layout row justify-center>
      <v-dialog v-model="dialog" persistent max-width="400">
        <v-card>
        <v-card-title class="headline">Chi tiết ghi nhận</v-card-title>
        <v-card-text>
          <p><strong>Thời gian ghi nhận: </strong>{{ selectedTinhtrang.ngayghinhan }}</p>
          <p><strong>Người ghi: </strong>{{ selectedTinhtrang.tenkythuatvien != null ? selectedTinhtrang.tenkythuatvien : selectedTinhtrang.tengiangvien }}</p>
          <p><strong>Nội dung: </strong>{{ selectedTinhtrang.mota_kythuatvien != null ? selectedTinhtrang.mota_kythuatvien : selectedTinhtrang.mota_giangvien }}</p>
        </v-card-text>
        <v-card-actions>
            <v-spacer></v-spacer>
...
export default {
  ...
  created: function() {
    ...
    _this.sothutumay = _this.may_slug.replace('p','P');
    _this.sothutumay = _this.sothutumay .replace('-','_');
  ...
  methods: {
    ...
    viewItem (item) {
      this.dialog = true;
      this.selectedTinhtrang = item;
      //this.$router.push({ path: `/admin/may/tinhtrangmay/${this.may_slug}/view/${id}` });
    },
    deleteItem (item,id) {
